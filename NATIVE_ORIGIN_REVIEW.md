...
## Android compilation default and optional live probe

`field-app-android` now compiles the release AAB and debug APK without requesting production MC by default. It still downloads build dependencies and embeds the existing protected CF settings. An always-run validation step requires both `CF_CLIENT_ID` and `CF_CLIENT_SECRET` before native generation; skipping the probe does not permit empty settings. The workflow does not install/launch the app or publish to Play.

Leave `MC_HEALTH_PROBE` unset or set to `0` for the compilation-only MC boundary. The step prints an explicit skipped message. Setting the build override `MC_HEALTH_PROBE=1` opts into one HTTPS GET to `https://automation.509electric.com/api/ping` with those gate headers. This is a real production read, not synthetic testing. Other nonempty values fail closed. The opted-in check requires HTTP200 and the actual public MC ping shape `{ok:true,t:<positive integer>,uptime:<finite nonnegative number>}`, confirmed from the live source at `server.mjs:2713` and staged source at `server.mjs:2858`. Logs contain only validated status and fixed diagnostics, never response headers/body or credential values. The private temporary response is deleted. `/api/ping` is explicitly public before the MC API auth gate. The former `/api/health` requires MC authentication and is unsuitable for a CF-only HTTP200 check. This opt-in probe establishes public origin reachability only; it does not prove named login, gate-credential validity, service health or native behavior.

The first hosted-build proposal uses the exact reviewed successor commit, `field-app-android`, and `MC_HEALTH_PROBE=0`. It consumes hosted quota and produces artifacts; it does not activate MC152, launch against the old production app, or establish native device/credential validity. `ios-simulator-smoke` still launches the configured production app and `ios-testflight` still publishes to TestFlight; neither is included in that compilation-only proposal.

An earlier unguarded test-harness draft unintentionally requested production `/api/ping` using synthetic CF header values and observed HTTP302. No redirect-follow option was used. The exact timestamp and request count were not retained; the separate incident receipt records that limitation and the safe observed diagnostic. No real credential values or raw response content were present in retained output. The final tests validate fixture-first curl resolution before executing the step in a minimal environment, fail before any call if that guard fails, and use only the fake transport. The entire repair must not be described as network-free; only the final guarded verification is.
